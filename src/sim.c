...
char *getBinary(unsigned int num)
{
    ...
    bstring = (char*) malloc(sizeof(char) * 6);
    assert(bstring != NULL);

    bstring[5] = '\0';

    for( i = 0; i < 5; i++ ) {
        bstring[5 - 1 - i] = (num == ((1 << i) | num)) ? '1' : '0';
    }
...
char *formatBinary(char *bstring)
{
    ...
    formatted = (char *) malloc(sizeof(char) * 8);
...
int main(int argc, char **argv)
{
    ...
    while( fgets(buffer, LINELENGTH, file) != NULL ) {
        if(buffer[0] != '#') {
            ...
            if(DEBUG) printf("counter %i:mode%c address%s\n", counter, mode, address);
...
int readFromCache(Cache cache, char* address)
{
	...
	if(DEBUG){
		if (CACHE_SIZE == 16){
			 printf("Cache Read Hit, Block %i %i\n", btoi(tag), btoi(index));
		}
		else if(CACHE_SIZE == 32){
			if(DEBUG) printf("Cache Read Hit, Block %s\n", index);
		}
		else{
			printf("Input Cache Size of Problem 13 is wrong!");
		}
	}
        cache->hits++;
        free(tag);
	...
	if(DEBUG){
		if (CACHE_SIZE == 16){
			 printf("Cache Read Miss, Block %i %i\n", btoi(tag), btoi(index));
		}
		else if(CACHE_SIZE == 32){
			if(DEBUG) printf("Cache Read Miss, Block %s\n", index);
		}
		else{
			printf("Input Cache Size of Problem 13 is wrong!");
		}
...
int writeToCache(Cache cache, char* address)
{
	...
	if(DEBUG){
		if (CACHE_SIZE == 16){
			 printf("Cache Write Hit, Block %i %i\n", btoi(tag), btoi(index));
		}
		else if(CACHE_SIZE == 32){
			if(DEBUG) printf("Cache Write Hit, Block %s\n", index);
		}
		else{
			printf("Input Cache Size of Problem 13 is wrong!");
		}
	...
	if(DEBUG){
		if (CACHE_SIZE == 16){
			 printf("Cache Write Miss, Block %i %i\n", btoi(tag), btoi(index));
		}
		else if(CACHE_SIZE == 32){
			if(DEBUG) printf("Cache Write Miss, Block %s\n", index);
		}
		else{
			printf("Input Cache Size of Problem 13 is wrong!");
		}
